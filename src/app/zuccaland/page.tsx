...
function ZuccalandTicketBuyer() {
  ...
  const handleCheckDiscount = async () => {
    ...
    try {
      const res = await fetch(`/api/discounts?code=${discountCode.trim().toUpperCase()}`);
      const data = await res.json();
      if (res.ok && data.discount) setDiscountData(data.discount);
...
// ─── Main Page ─────────────────────────────────────────────────────────────────
export default function ZuccalandPage() {
        ...
        <div style={{ position: 'relative', zIndex: 10 }}>
          ...
          >
            ...
            <p style={{
              ...
              Il villaggio delle zucche di Gasperina
            </p>
          </motion.div>
          ...
          >
            <div style={{
              ...
              fontSize: '1.1rem', fontWeight: 900, marginTop: '3rem',
              boxShadow: '0 10px 30px rgba(0,0,0,0.05), inset 0 -4px 0 rgba(234,88,12,0.1)',
